...
在注册一个组件的时候，始终需要给它一个名字。比如在全局注册的时候已经看到了：
...
给予组件的名字可能依赖于打算拿它来做什么。当直接在 DOM 中使用一个组件（而不是在字符串模板或单文件组件）的时候，强烈推荐遵循 W3C 规范中的自定义组件名（字母全小写且必须包含一个连字符）。这会避免和当前以及未来的 HTML 元素相冲突。
...
```js
var componentA = { template: '<h3>hi</h3>' }
...
通过 `import`/`require` 使用一个模块系统的特殊的使用说明和注意事项。

##### 在模块系统中局部注册

如果使用了诸如 Babel 和 webpack 的模块系统。在这些情况下，推荐创建一个 `components` 目录，并将每个组件放置在其各自的文件中。
...
可能许多组件只是包裹了一个输入框或按钮之类的元素，是相对通用的。有时候会把它们称为基础组件，它们会在各个组件中被频繁的用到。
...
如果恰好使用了 webpack（或在内部使用了 webpack 的 Vue CLI 3+），那么就可以使用 `require.context` 只全局注册这些非常通用的基础组件。这里有一份可以在应用入口文件（比如 `src/main.js`）中全局导入基础组件的示例代码：
...
但是，通常希望每个 prop 都有指定的值类型。这时，可以以对象形式列出 prop，这些 property 的名称和值分别是 prop 各自的名称和类型：
...
可以为组件的 prop 指定验证要求，例如知道的这些类型。如果有一个需求没有被满足，则 Vue 会在浏览器控制台中警告。这在开发一个会被别人用到的组件时尤其有帮助。
...
例如，想象一下通过一个 Bootstrap 插件使用了一个第三方的 `<bootstrap-date-input>` 组件，这个插件需要在其 `<input>` 上用到一个 `data-date-picker` attribute。可以将这个 attribute 添加到组件实例上：
...
```html
<!--
如果不设置 `inheritAttrs: false`，父组件传递的 required 和 placeholder attribute 会添加到根元素即 label 上，因为格式为：
	<label>
		<input>
	</label>
-->
...
<!--
如果设置了 `inheritAttrs: false`，则父组件传递的 required 和 placeholder attribute 只会添加到设置了 `$attrs` property 的元素上
-->
<label>
	Username:
	<input required="required" placeholder="Enter your username">
